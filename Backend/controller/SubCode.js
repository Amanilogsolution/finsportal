...
const InsertGlSubCode = async (req, res) => {
    ...
    try {
        ...
        const result = await sql.query(`INSERT into ${org}.dbo.tbl_gl_sub(charge_code,sub_code,gl_code,company_id,add_user_name,add_system_name,
            ...
            values('${charge_code}','${SubCode}','${glCode}','${company_id}','Aman','${os.hostname()}','${req.ip}',getDate(),'Active')`)
            res.send(result.recordset)
    }
    catch (err) {
        res.send(err)
...
const UpdateSubCodeDetails = async (req, res) => {
    ...
    try {
        await sql.connect(sqlConfig)
        const result = await sql.query(`update ${org}.dbo.tbl_gl_sub set charge_code='${charge_code}',sub_code='${sub_code}',gl_code='${gl_code}',company_id='Awl',
        update_user_name='Rupesh',update_system_name='${os.hostname()}',update_ip_address='${req.ip}',update_date_time=GETDATE() WHERE sno=${sno}`)
        res.send(result.recordset[0])
    }
    catch (err) {
        res.send(err)
...
module.exports = {GlCode,GlSubCode,InsertGlSubCode,ShowTotalSubCode,SubCodeStatus,GetSubCodeDetails,UpdateSubCodeDetails}
