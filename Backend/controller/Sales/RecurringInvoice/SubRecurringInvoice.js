...
const InsertSubRecurringInvoice = async (req, res) => {
...
const UpdateSaveSubRecurringInvoice = async (req, res) => {
    const org = req.body.org;
    const invoice_no = req.body.invoice_no;
    const new_invoice_no = req.body.new_invoice_no;
    try {
        await sql.connect(sqlConfig)
        const result = await sql.query(` update ${org}.dbo.tbl_recurring_subinvoice set invoice_no='${new_invoice_no}'  WHERE invoice_no='${invoice_no}'`)
