...
import { AllAccountInfo, AccountInfoStatus,Importaccountinfo } from '../../../api';
import * as XLSX from "xlsx";
import Excelfile from '../../../excelformate/Item_account_info.xlsx';
...
function ShowAccountInfo() {
    ...
    const uploaddata = async () => {
        ...
        importdata.map((d) => {
          if (!d.account_info_name || !d.account_info_type) {
            setErrorno(errorno++);
          }
        })
        ...
        if (errorno > 0) {
            ...
            console.log(importdata)
              const result = await Importaccountinfo(importdata,localStorage.getItem("Organisation"),localStorage.getItem("User_id"));
                            ...
                            <div className="modal-content">
                                ...
                                <div className="" style={{ margin: "auto", paddingBottom: "20px", overflow: "auto" }}>
                                    <table >
                                        <thead>
                                            <tr>
                                                <th style={{ border: "1px solid black" }}>account_info_name</th>
                                                <th style={{ border: "1px solid black" }}>account_info_type</th>
                                        ...
                                        <tbody>
                                            {
                                                importdata.map((d) => (
                                                    <tr style={{ border: "1px solid black" }}>
                                                        <td style={{ border: "1px solid black" }}>{d.account_info_name}</td>
                                                        <td style={{ border: "1px solid black" }}>{d.account_info_type}</td>
