...
import './bill.css'
import { ActiveVendor, ActiveSelectedVendor, ActivePurchesItems, Activeunit, ActivePaymentTerm, SelectVendorAddress, Getfincialyearid, InsertBill, ActiveUser, ActiveLocationAddress, InsertSubBill, Updatefinancialcount, UploadData, GetPodetailsVendor } from '../../../api'
            ...
            if (bill_amt == total_bill_amt) {
                ...
                if (result == 'Added') {
                    amount.map(async (amt, index) => {
                        const result1 = await InsertSubBill(org, voucher_no, voucher_date, bill_date, bill_no, vendor_id, vendor_name, location[index], items[index], employee[index], 'glcode', 'samt', quantity[index],
                                                ...
                                                totalValues.map((element, index) => (
                                                    <tr key={index}>
                                                        ...
                                                        <td className='p-1 pt-2' style={{ width: "150px" }}>
                                                            <input type='number' id={`deduction${index}`} className="form-control" defaultValue='0' />
