...
import {
  ActiveAllChartofAccount,
  SearchActiveChartofAccount,
  showOrganisation,
  ActiveBank,
  ActiveVendor,
  ...
  Getfincialyearid,
  InsertBillPayment,
  Updatefinancialcount,
  InsertSubBillPayment,
} from "../../../api";
...
function AddBankingPayment() {
  ...
  useEffect(() => {
    const fetchdata = async () => {
      ...
      setLoading(true);
      document.getElementById("bank_recep_id").value =
        id[0].bank_payment_ser + id[0].year + String(lastno).padStart(5, "0");
      Todaydate();
    };
  ...
  const handleClickVendor = async (id, name, mast_id) => {
    const onAccount =
      document.getElementById("on_account").checked === true ? true : false;
    if (!onAccount) {
      const bills = await GetBillVendorID(
  ...
  const handleSetBillData = (index, bill_no, bill_date, bill_amt, location) => {
    const getcheckval =
      document.getElementById(`billcheck-${index}`).checked === true
        ? true
        : false;
    ...
    if (getcheckval) {
      setSelectedBillData([
        ...selectedBillData,
        {
          chart_of_acct: bankPayMinData[currentindex].chart_of_acct,
          achead: bankPayMinData[currentindex].achead,
          glcode: bankPayMinData[currentindex].glcode,
          vendorId: bankPayMinData[currentindex].vendorId,
          masterId: bankPayMinData[currentindex].masterId,
          costCenter: location,
          refNo: bill_no,
          refDate: bill_date,
          refAmt: bill_amt,
  ...
  const handleSubmitForm = async () => {
    ...
    if (!check_ref_no || !check_amt || !bank_id) {
      ...
      bankPayMinData.map(async (element) => {
        const result = await InsertSubBillPayment(
          org,
          bank_payment_id,
          element.glcode,
          element.achead,
      ...
      if (result.result === "Added successfully") {
        setAlertObj({
          type: "success",
          text: "Bank Payment Done",
          url: "/TotalBankingPayment",
        });
      } else {
              ...
              <div className="card">
                <article className="card-body">
                  <form autoComplete="off">
                    <div className="form-row ">
                      ...
                      <label
                        htmlFor="bank_recep_date"
                        className="col-md-2 col-form-label font-weight-normal"
                      >
                        {" "}
                        Banking Receipt Date
                      </label>
                      <div className="d-flex col-md-4">
                        <input
                          type="date"
                    ...
                    <div className="form-row mt-2">
                      <label
                        htmlFor="check_ref_no"
                        className="col-md-2 col-form-label font-weight-normal"
                      >
                        Cheque no/Ref no <span className="text-danger">*</span>
                      </label>
                      <div className="d-flex col-md-4">
                        {" "}
                        <input
                    ...
                    <div className="form-row mt-2">
                      <label
                        ...
                        className="col-md-2 col-form-label font-weight-normal"
                      >
                        Cheque amt <span className="text-danger">*</span>
                      </label>
                      <div className="d-flex col-md-4">
                        {" "}
                        <input
                      ...
                      <div className="d-flex col-md-4">
                        ...
                        >
                          ...
                          {banklist.map((bankdata, index) => (
                            ...
                            >
                              {" "}
                              {bankdata.bank_name} ({bankdata.account_no}){" "}
                            </option>
                          ))}
                        </select>
                      </div>
                    ...
                    <div className="form-row">
                      ...
                      <div className="d-flex col-md-4 pt-2">
                        <input
                          type="checkbox"
                          id="on_account"
                    ...
                    <input
                      ...
                      value="Add Row"
                      onClick={handleAddMinorDataRow}
                    />
                    <input
                      type="button"
                      className="btn btn-danger ml-2"
                      value="Remove"
                      onClick={(e) => handleRemoveDeleteRow(e, "pop")}
                    />
                ...
                <div className="card-footer border-top">
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      window.location.href = "/TotalBankingPayment";
                    }}
                    name="clear"
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              <table className="table  table-striped table-sm ">
                ...
                >
                  <tr>
                    <th className="pl-4 text-left" style={{ fontSize: "20px" }}>
                      Select
                    </th>
                    <th className="pl-4 text-left" style={{ fontSize: "20px" }}>
                      Bill no
            ...
            <div className="modal-header">
              ...
              <div className="form-group col-md-5">
                <input
                  ...
                  id="searchChartofAcct"
                  onChange={handleSearchChartofAccount}
                />
              </div>
            </div>
            <div
              className="modal-body overflow-auto px-5 pt-0"
                ...
                <tbody>
                  {chartofacctlist.map((items, index) => (
                    <tr
                      ...
                      onClick={(e) =>
                        handleChnageAcHead(
                          items.account_sub_name,
                          items.account_sub_name_code
                        )
                      }
                    >
                      <td>{index + 1}</td>
                  ...
                  {locationstate.length > 0
                    ? locationstate.map((items, index) => (
                        <tr
                          ...
                          data-dismiss="modal"
                          onClick={(e) => {
                            handlelocation(items.location_id);
                          }}
                        >
                          <td>{items.location_city}</td>
                          <td style={{ fontSize: "15px" }}>
                            {items.location_add1},{items.location_city},
                            {items.location_country}
                          </td>
                        </tr>
                      ))
                    : "Select Customer"}
